...
generatePackages() {
    ...
    xz -cv $BD/system-treble_arm64_bvN.img -T0 > $BD/PixysOS_arm64-ab-13.0-$buildDate-UNOFFICIAL.img.xz
    xz -cv $BD/system-treble_arm64_bvN-vndklite.img -T0 > $BD/PixysOS_arm64-ab-vndklite-13.0-$buildDate-UNOFFICIAL.img.xz
    xz -cv $BD/system-treble_arm64_bvN-slim.img -T0 > $BD/PixysOS_arm64-ab-slim-13.0-$buildDate-UNOFFICIAL.img.xz
    rm -rf $BD/system-*.img
    echo
}
...
generateOta() {
    ...
    find $BD/ -name "PixysOS_*" | sort | {
        while read file; do
            ...
            url="https://github.com/changanmoon/treble_build_pixys/releases/download/$version/$filename"
